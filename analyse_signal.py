...
def my_visu(sf, sphere, rot=True, norm=True, radial_scale=True, scale=0.5, title="Modeling"):
    ren = window.Scene(background=window.colors.white)
    
    sf_actor = actor.odf_slicer(sf[None, None, None, :], sphere=sphere, colormap='jet',
                                norm=norm, radial_scale=radial_scale, scale=scale)
    if rot :
        sf_actor.RotateX(90)
...
img = nib.load("single_voxel_dwi_MTR.nii.gz")
data = img.get_fdata(dtype=np.float32)
affine = img.affine
bvals, bvecs = read_bvals_bvecs("dwi_mt_off.bval", "dwi_mt_off.bvec")
...
print(gtab.bvals)
print(gtab_vis.bvals)

# on prend un voxel du signal dans le vert
signal = data.reshape(31,)
...
my_visu(signal_sph, sph_gtab_vis, rot=False, norm=True, radial_scale=True, title="Raw signal")
...
my_visu(signal_sphere, sphere, rot=False, norm=False, radial_scale=False, title="Raw signal")
