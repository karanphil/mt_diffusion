...
def _build_arg_parser():
    ...
    p.add_argument('--mtr', nargs='+', required=True,
                   help='List of images to visualize.')
    
    p.add_argument('--fixel_mtr', nargs='+', required=True,
                   help='List of images to visualize.')
...
def main():
    ...
    mtr, _ = extract_measures(args.mtr, data_shape)
    mtr = np.ma.masked_where(mtr == 0, mtr)

    fixel_mtr, _ = extract_measures(args.fixel_mtr, data_shape)
    ...
    vmax = np.array([0.5, 0.5, 0.5, 0.5, 0.5, 0.5])
    ...
    if args.combine_colorbar:
        fig, ax = plt.subplots(mtr.shape[-1], 6,
                               gridspec_kw={"width_ratios":[1.0, 1.1, 0.8, 1.0, 1.1, 0.8]},
                               layout='constrained')
    else:
        fig, ax = plt.subplots(mtr.shape[-1], 7,
