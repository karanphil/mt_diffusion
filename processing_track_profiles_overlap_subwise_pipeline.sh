    ...
    for b in $bundles;
        ...
        for c in ${cross_bundles[$count]};
            ...
            if [ ! -f "fixel_mtr_profile_${b}_crossing_${c}.txt" ]; then
                ...
                crossing_bundle_afd="${register_data_dir}/output/results_registration/${sub}/Metrics_into_template_space/afd_fixel_${c}_to_template.nii.gz";

                python ${code_dir}/python_scripts/compute_track_profiles.py $mtr $fixel_mtr $labels ${b} $afd_fixel $nufo . --in_bundle_map $bundle_mask --map_threshold $map_thr --afd_threshold $afd_thr --min_nvox $min_nvox --nb_sections $nb_sections --in_crossing_bundle_map $crossing_bundle_mask --in_crossing_bundle_afd $crossing_bundle_afd --in_crossing_bundle_name ${c} -f;
